                    ...
                    <Box sx={{ mt: 2 }}>
                      ...
                      <Grid container spacing={1}>{images.map((image, index) => (
                        <Grid item key={index} xs={4} sm={2} md={1}>
                          <Paper sx={{ position: 'relative', width: { xs: 64, sm: 80 }, height: { xs: 64, sm: 80 }, overflow: 'hidden' }}>
                            <img src={URL.createObjectURL(image)} alt={`Property ${index + 1}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                            <IconButton size="small" sx={{ position: 'absolute', top: 4, right: 4, backgroundColor: 'rgba(0,0,0,0.5)', color: 'white', '&:hover': { backgroundColor: 'rgba(0,0,0,0.7)' } }} onClick={() => setImages(prev => prev.filter((_, i) => i !== index))}>
